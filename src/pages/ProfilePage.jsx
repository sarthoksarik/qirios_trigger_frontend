...
const ProfilePage = () => {
  ...
  const [initialSelectionProcessed, setInitialSelectionProcessed] =
    useState(false);
  ...
  useEffect(() => {
    console.log(
      `ProfilePage Effect 3 (Update Check): Processed=${initialSelectionProcessed}, TriggeredRef=${hasTriggeredInitialUpdate.current}, SelectedDID=${selectedCustomer?.did_number}, URL_DID=${customerId}`
    );
    ...
    if (
      initialSelectionProcessed &&
      selectedCustomer?.did_number === customerId &&
      !hasTriggeredInitialUpdate.current
    ) {
      console.log(
        `ProfilePage Effect 3: Triggering initial update for ${customerId}.`
      );
