...
const AppProvider = ({ children }) => {
  ...
  const updateSelectedCustomerFromSheet = useCallback(async () => {
    ...
    try {
      ...
      try {
        console.log("Refreshing full customer list...");
        const fullRefreshResponse = await apiClient.get("/customers/");
        const updatedCustomers = Array.isArray(fullRefreshResponse.data)
          ? fullRefreshResponse.data
          : [];

        setCustomers(updatedCustomers);

        // Reselect the current customer by DID from the updated list
        const reselectedCustomer = updatedCustomers.find(
          (c) => c.did_number === selectedCustomer.did_number
        );

        if (reselectedCustomer) {
          setSelectedCustomer(reselectedCustomer);
          setSelectedTitle(null);
          setSelectedDemand(null);
          setSelectedPatientType(null);
          clearCurrentActions();
          console.log("Customer reselected and data refreshed successfully.");
        } else {
          setSelectedCustomer(null);
          setError("Customer not found in refreshed data.");
        }
      } catch (refreshErr) {
        console.error(
          "Error refreshing customer list after update:",
          refreshErr
        );
        setUpdateError(
          "Update succeeded, but failed to refresh full customer list. Please reload manually."
        );
      }
